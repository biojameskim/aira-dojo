...
from dojo.core.solvers.operators.judge import judge_op
...
def puct_value(
    ...
    global_max_q_val: float,
    global_min_q_val: float,
    p_action_state: float,
):
    """
    Compute PUCT (Predictor + Upper Confidence Bound) value for action selection.

    Formula: Q(s,a) + c_puct * P(s,a) * sqrt(∑_b N(s,b)) / (1 + N(s,a))

    Args:
        q_value: Mean action value Q(s,a)
        explore_count: Visit count N(s,a) for this action
        parent_explore_count: Total visit count ∑_b N(s,b) for parent state
        uct_c: Exploration constant c_puct
        global_max_q_val: Global maximum Q value for normalization
        global_min_q_val: Global minimum Q value for normalization
        p_action_state: Prior probability P(s,a) from policy

    Returns:
        PUCT value for action selection (higher is better)
    """
    if explore_count == 0:
        return -1e8
    ...
    # PUCT exploration: c_puct * P(s,a) * sqrt(parent_visits) / (1 + child_visits)
    exploration = uct_c * p_action_state * math.sqrt(parent_explore_count) / (1 + explore_count)
...
class MCTSNode(Node):
    explore_count: int = 0
    node_value: float = 0
    prior: float = 0.0  # Prior probability from judge (0.0 = uniform default)
...
class MCTS(Solver):
    def __init__(self, cfg: MCTSSolverConfig, task_info):
        ...
        # Debug log to confirm the PMCTS solver is being instantiated
        self.logger.info(f"Initialized PMCTS solver from {__file__}")
    ...
    def setup_operators(self):
        ...
        This method instantiates the draft, improve, debug, analyze, and judge LLMs
        ...
        judge_llm = GenericLLM(self.cfg.operators["judge"])
        ...
        self.judge_fn = partial(judge_op, judge_llm, self.cfg, self.memory_op)
    ...
    def search_policy(self, root_node: MCTSNode) -> List[MCTSNode]:
        """
        Traverse the tree from root to leaf using PUCT selection.
        ...
        while True:
            ...
            # Lazy judging: upgrade uniform priors to informed priors on first revisit
            self._ensure_children_judged(current_node)

            current_node = max(
                current_node.children,
                key=lambda c: puct_value(
    ...
    def _judge_children(
        ...
        Args:
            parent_node: Parent node being expanded
        ...
        # Parse JSON response
        judgment = parse_json_output(judgment_json)
        ...
        if prior_sum > 0:
            ...
            priors = [1.0 / len(children)] * len(children)
    ...
    def _ensure_children_judged(self, node: MCTSNode):
        """
        Upgrade children's uniform priors to LLM-judged priors (once per parent).

        This method is called lazily during search_policy to assign informed priors
        to children that were initially created with uniform priors.

        Args:
            node: Parent node whose children need judging
        """
        if not node.children:
            return

        # Check if already judged (store flag on parent node)
        if hasattr(node, '_children_judged') and node._children_judged:
            return
        ...
        # Mark as judged to avoid re-judging
        node._children_judged = True
        self.logger.info(f"Node {node.id} children judged: {len(children_list)} children")
    ...
    def _expand_leaf_and_backprop(self, path: List[MCTSNode], state: Any, task: Any) -> Tuple[Any, int]:
        ...
        for _ in range(num_children_to_create):
            ...
            # Assign uniform prior (will be upgraded to informed prior on next visit)
            child_node.prior = 1.0 / num_children_to_create
